<template>
  <main class="overflow-y-auto bg-gray-50 w-full">
    <Head title="Водяной знак" />

    <div class="grid mx-auto relative">
      <div
        class="absolute w-full h-full bg-white z-50 px-2"
        id="loader"
        v-if="this.$store.state.loader == true"
      >
        <div class="text-3xl font-medium">
          <span class="text-orange">Подождите.</span> Преобразуем текст в
          изображение...
        </div>
      </div>
      ...
      >
        <div class="flex items-center flex-row pt-3 py-4 z-10">
          ...
          <template v-if="menu_trigger == true">
            <div class="text-md flex items-center flex-row">
              <div
      ...
      <div class="h-full">
        <!-- <watermark-label name="alive" class="w-4" /> тут векторная метка...еще надо написать в редакторе-->

        <div class="flex-center position-ref p-6" v-show="tab == 'editor'">
          <div class="wrapp_photo-container inline-block">
            <div
              id="photo-container"
              v-if="$store.state.watermark_image"
              class="relative"
            >
              <div
                class="w-full h-full bg-gray-400 absolute opacity-50 z-10"
                v-if="$store.state.label.on == true"
              ></div>
              <div
                class="flex flex-wrap content-center justify-center absolute top-0 left-0 h-full w-full"
              >
                <!-- alive -->
                <div
                  class="label_template_alive she_found_alive z-20"
                  v-show="$store.state.label.female.alive == true"
                >
                  Найдена. Жива
                </div>
                <div
                  class="label_template_alive he_found_alive z-20"
                  v-show="$store.state.label.male.alive == true"
                >
                  Найден. Жив
                </div>
                <div
                  class="label_template_alive all_found_alive z-20"
                  v-show="$store.state.label.all.alive == true"
                >
                  Найдены. Живы
                </div>
                ...
                <!-- died -->
                <div
                  class="label_template_died she_found_died z-20"
                  v-show="$store.state.label.female.died == true"
                >
                  Найдена. Погибла
                </div>
                <div
                  class="label_template_died he_found_died z-20"
                  v-show="$store.state.label.male.died == true"
                >
                  Найден. Погиб
                </div>
                <div
                  class="label_template_died all_found_died z-20"
                  v-show="$store.state.label.all.died == true"
                >
                  Найдены. Погибли
                </div>
              </div>
              ...
              <img
                :src="$store.state.watermark_image"
                class="img-responsive inline-block"
                v-if="
                  $store.state.label.on == false &&
                  $store.state.label.male.alive == false &&
                  $store.state.label.female.alive == false &&
                  $store.state.label.male.died == false &&
                  $store.state.label.female.died == false &&
                  $store.state.label.all.alive == false &&
                  $store.state.label.all.died == false
                "
              />
              <img
                :src="$store.state.watermark_image"
                ...
                v-if="
                  $store.state.label.male.alive == true ||
                  $store.state.label.female.alive == true ||
                  $store.state.label.all.alive == true
                "
              />
              <img
                :src="$store.state.watermark_image"
                ...
                v-if="
                  $store.state.label.male.died == true ||
                  $store.state.label.female.died == true ||
                  $store.state.label.all.died == true
                "
              />
        ...
        <div class="flex-center position-ref p-6" v-show="tab == 'explorer'">
          <div class="wrapp_photo-container inline-block inline-block">
            <div id="image-result">
              <img
                src="https://fakeimg.pl/350x350/"
                alt="resultImage"
                id="viewImage"
              />
            </div>
...
import WatermarkLayout from "@/Shared/Watermark/Layout";
import WatermarkLabel from "@/Shared/WatermarkLabel";
...
export default {
  ...
  mounted() {
    if (localStorage.watermark_image) {
      this.$store.commit("returnImageLabel", localStorage.watermark_image);
    }
  },
  created: function () {
  ...
  methods: {
    converter() {
      ...
      htmlToImage
        .toPng(document.getElementById("photo-container"), {
          imagePlaceholder: "https://fakeimg.pl/350x350/",
        })
        .then(function (dataUrl) {
          ...
          document.getElementById("image-result").append(img);
        })
    ...
    onImageChange(e) {
      ...
      this.createWatermarkImage(files[0]);
    },
    createWatermarkImage(file) {
      let reader = new FileReader();
      let self = this;

      reader.onload = (e) => {
        console.log(e.target);
        self.$store.commit("createWatermarkImage", e.target.result);
        localStorage.watermark_image = e.target.result;
      };
      reader.readAsDataURL(file);
    },
    removeImageLabel() {
      this.$store.commit("removeWatermarkImage");
    },
    ...
    downloadImage() {
      ...
      let url = document.querySelector("#image-result img").src;
