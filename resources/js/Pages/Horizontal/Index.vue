<template>
  <div class="overflow-y-auto bg-gray-50 w-full">
    <Head title="Горизонтальная ориентация" />

    <div
      class="flex items-center justify-between w-full text-sm bg-white border-b-4 border-solid px-12 py-0"
    >
      <div class="flex items-center flex-row pt-3 py-4 z-10">
        <div class="mr-4">
          <button
            class="menu-trigger"
            :class="{ active: menu_trigger }"
            id="menu01"
            @click="menuTrigger"
          >
            <span></span>
        ...
        <template v-if="menu_trigger == true">
          <div class="flex items-center flex-row text-md">
            <div
              @click="visible('editor')"
              class="flex items-center flex-row content-center py-2 px-3 mr-4 rounded-md bg-pink_flambe text-white cursor-pointer"
            >
              <svg
                version="1.1"
                class="mr-2"
              ...
              <div class="">Редактор</div>
            </div>

            <div
              @click="visible('explorer')"
              class="flex items-center flex-row content-center py-2 px-3 mr-4 rounded-md bg-very_peri text-white cursor-pointer"
            >
              <svg
                version="1.1"
                class="mr-2"
              ...
              <div class="">Просмотр и скачивание</div>
            </div>
          </div>
        </template>

        <div
          class="mr-4 flex flex-wrap content-center text-gray-800 text-3xl font-bold text-gray-800"
          v-if="menu_trigger == false && menu_editor == true"
        >
          <div>Редактор:</div>
        </div>

        <div
          class="flex items-center flex-row mr-4 content-center py-2 px-3 rounded-md bg-orange_c text-white cursor-pointer"
          v-if="menu_trigger == false && menu_editor == true"
          @click="converter()"
        >
          <svg
          ...
          <div class="">Преобразовать в изображение формата *.PNG</div>
        </div>

        <div
          class="flex items-center flex-row content-center mr-4 text-gray-800 text-3xl font-bold"
          v-if="menu_trigger == false && menu_explorer == true"
        >
          <div>Просмотр и скачивание:</div>
        </div>
        ...
        <div
          class="flex items-center flex-row mr-4 content-center py-2 px-3 rounded-md bg-very_peri text-white cursor-pointer"
          v-if="menu_trigger == false && menu_explorer == true"
          @click="downloadImage()"
        >
          <svg
          ...
          >
            <g>
              <g>
                <path
                  fill="#fff"
                  d="M0,348.65v84.2c0,28.9,23.5,52.5,52.5,52.5h384.4c28.9,0,52.5-23.5,52.5-52.5v-84.2c0-28.9-23.5-52.5-52.5-52.5h-106
                ...
                <path
                  fill="#fff"
                  d="M244.7,4.05c-6.8,0-12.3,5.5-12.3,12.3v209.8l-59.3-59.3c-4.8-4.8-12.5-4.8-17.3,0s-4.8,12.5,0,17.3l80.2,80.2
          ...
          <div class="mt-2 py-2 text-sm bg-white rounded shadow-xl">
            ...
            <a
              ...
              ><el-icon size="25" class="mr-2" color="#0088cc"
                ><Promotion
              /></el-icon>
              <span>Написать в телегам</span></a
            >
          </div>
        </template>
      </dropdown>
    ...
    <div class="h-full">
      <div class="flex-center position-ref p-6" v-show="tab == 'editor'">
        <div id="form-horizontal" class="form-orientation block">
          <div class="level_1">
            <div id="editor-top" class="editor-top">
              <div contenteditable="true" @input="onInputCaptionTop">
                {{ $store.state.caption.top }}
              </div>
            </div>
            <div
              class="level_2"
              :style="{ 'border-color': this.$store.state.color }"
            >
              <div class="content flex flex-col">
                <div class="info flex flex-wrap justify-between w-full">
                  <div class="col-1 p-0 m-0 relative">
                    <div
                      id="photo-container"
                      v-if="$store.state.photo"
                      :style="{ 'border-color': this.$store.state.color }"
                      class=""
                    >
                      <img
                    ...
                    <div
                      v-else
                      class="img border-4 border-red-600"
                      :style="{ 'border-color': this.$store.state.color }"
                    >
                    ...
                    <div class="notification">
                      ...
                      <div
                        class="item_3"
                        :class="'mod_' + $store.state.search.selected_id"
                      >
                        {{ $store.state.search.selected_label }}
                      </div>
                    </div>
                  </div>
                  <div class="col-2 p-0 m-0 flex flex-col justify-between">
                    <div class="editor">
                      <quill-editor
                        theme="bubble"
                        id="BodyEditor"
                        ...
                        @ready="onBodyEditorReady($event)"
                        @focus="onBodyEditorFocus($event)"
                        @blur="onBodyEditorBlur($event)"
                        @update:content="onBodyEditorChange($event)"
                      />
                    </div>
                    <div
                      class="footer"
                    ...
                    >
                      <quill-editor
                        ...
                        :options="editorOption"
                        placeholder="Lorem ipsum..."
                        @ready="onFooterEditorReady($event)"
                        @focus="onFooterEditorFocus($event)"
                        @blur="onFooterEditorBlur($event)"
                        @update:content="onFooterEditorChange($event)"
                      />
                    </div>
                  </div>
                </div>
            ...
            <div id="editor-bottom" class="editor-bottom">
              <div contenteditable="true">
                {{ $store.state.caption.bottom }}
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex-center position-ref p-6" v-show="tab == 'explorer'">
        <div id="image-horizontal">
          <img
            src="https://fakeimg.pl/1220x860/"
            alt="resultImage"
            id="viewImage"
          />
        </div>
...
import HorizontalLayout from "@/Shared/Horizontal/Layout";
...
export default {
  name: "horizontal_oriental",
  title: "Горизонтальная ориентация",
  components: {
    Head,
    QuillEditor,
    Dropdown,
  ...
  mounted() {
    ...
    if (!localStorage.caption_top) {
      localStorage.caption_top = this.$store.getters.getCaptionTopText;
    } else {
      this.$store.commit("changeCaptionTopText", localStorage.caption_top);
    }

    if (!localStorage.caption_bottom) {
      localStorage.caption_bottom = this.$store.getters.getCaptionBottomText;
    } else {
      this.$store.commit(
        "changeCaptionBottomText",
        localStorage.caption_bottom
      );
    }

    // if(!localStorage.horizontal_body){
    //   localStorage.horizontal_body = this.$store.getters.getHorizontalBodyText
    // } else {
    //   this.$store.commit('changeHorizontalBodyText', localStorage.horizontal_body);
    // }

    // if(!localStorage.horizontal_footer){
    //   localStorage.horizontal_footer = this.$store.getters.getHorizontalFooterText
    // } else {
    //   this.$store.commit('changeHorizontalFooterText', localStorage.horizontal_footer);
    // }
  },
  methods: {
    /* body */
    onBodyEditorReady(e) {
      if (!localStorage.horizontal_body) {
        e.container.querySelector("#BodyEditor .ql-blank").innerHTML =
          localStorage.horizontal_body =
            this.$store.getters.getHorizontalBodyText;
      } else {
        e.container.querySelector("#BodyEditor .ql-blank").innerHTML =
          localStorage.horizontal_body;
        this.$store.commit(
          "changeHorizontalBodyText",
          localStorage.horizontal_body
        );
      }
    },
    onBodyEditorFocus(e) {
      document.getElementById("BodyEditor").classList.add("EditorFocus");
    },
    onBodyEditorBlur(e) {
      document.getElementById("BodyEditor").classList.remove("EditorFocus");
    },
    onBodyEditorChange(e) {
      this.$store.commit("changeHorizontalBodyText", e);
    } /* end */,

    /* footer */
    onFooterEditorReady(e) {
      if (!localStorage.horizontal_footer) {
        e.container.querySelector("#FooterEditor .ql-blank").innerHTML =
          localStorage.horizontal_footer =
            this.$store.getters.getHorizontalFooterText;
      } else {
        e.container.querySelector("#FooterEditor .ql-blank").innerHTML =
          localStorage.horizontal_footer;
        this.$store.commit(
          "changeHorizontalFooterText",
          localStorage.horizontal_footer
        );
      }
    },
    onFooterEditorFocus(e) {
      document.getElementById("FooterEditor").classList.add("EditorFocus");
    },
    onFooterEditorBlur(e) {
      document.getElementById("FooterEditor").classList.remove("EditorFocus");
    },
    onFooterEditorChange(e) {
      this.$store.commit("changeHorizontalFooterText", e);
    } /* end */,

    converter() {
      const loading = ElLoading.service({
        lock: true,
        text: "Преобразуем текст в изображение...",
      ...
      htmlToImage
        .toPng(document.getElementById("form-horizontal"))
    ...
    onInputCaptionTop(e) {
      this.$store.commit("changeCaptionTopText", e.target.innerText);
    },
    onInputCaptionBottom(e) {
      this.$store.commit("changeCaptionBottomText", e.target.innerText);
    },
  },
  computed: {},
};
...
/*
      DejaVu Sans ExtraLight - 200
      DejaVu Sans Normal - 400
      DejaVu Sans Oblique - 400
      DejaVu Sans Bold - 700
      DejaVu Sans Bold Oblique - 700
  */
...
#form-horizontal .notification .item_3.mod_101 {
  /*человек*/
  font-size: 60px;
}
#form-horizontal .notification .item_3.mod_102 {
  /*ребенок*/
  font-size: 70px;
}
#form-horizontal .notification .item_3.mod_103 {
  /*подросток*/
  font-size: 52px;
}
#form-horizontal .notification .item_3.mod_104 {
  /*подростков*/
  letter-spacing: 0.06em;
  font-size: 44px;
  line-height: 65px;
}
#form-horizontal .notification .item_3.mod_105 {
  /*детей*/
  letter-spacing: 0.06em;
  font-size: 65px;
  line-height: 65px;
}
#form-horizontal .notification .item_3.mod_106 {
  /*родственников*/
  letter-spacing: 0.01em;
  font-size: 36px;
}
#form-horizontal .notification .item_3.mod_107 {
  /*людей*/
  letter-spacing: 0.06em;
  font-size: 65px;
  line-height: 65px;
